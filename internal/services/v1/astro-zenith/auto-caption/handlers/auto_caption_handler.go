...
import (
	...
	"app/internal/services/v1/astro-zenith/auto-caption/dto"
	"app/internal/services/v1/astro-zenith/auto-caption/models"
	"app/internal/services/v1/astro-zenith/auto-caption/repository"
	"net/http"
...
func UpdateVideo(c *gin.Context) {
	...
	auto_caption, err := repository.GetVideoByID(id)
	if err != nil {
		c.Error(errors.NewNotFound("auto_caption not found"))
		return
	}

	// Apply updates safely
	if req.Name != nil {
		auto_caption.Name = *req.Name
	}
	if req.Email != nil {
		auto_caption.Email = *req.Email
	}
	if req.Password != nil {
		auto_caption.Password = *req.Password
	}

	if err := repository.UpdateVideo(auto_caption); err != nil {
		c.Error(errors.NewInternal(utils.ParseDBError(err)))
		return
	}

	c.JSON(200, response.Success(200, "updated", dto.ToResponseDTO(auto_caption)))
}

func DeleteVideo(c *gin.Context) {
	id := c.Param("id")

	// Cek dulu apakah auto_caption ada
	auto_caption, err := repository.GetVideoByID(id)
	if err != nil || auto_caption == nil {
		c.Error(errors.NewNotFound("auto_caption not found"))
		return
	}
